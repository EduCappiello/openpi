...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        exp_name="openpi",
        project_name="B1K",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=32),  # full FT (gemma_2b)
        data=LeRobotB1KDataConfig(
...
def _b1k_wave_model():
    # FULL fine-tuning (no LoRA): paligemma_variant defaults to "gemma_2b", so the
    # whole 2B backbone + 300m action expert are trainable (get_freeze_filter() -> Nothing).
    # action_horizon=32 matches the monolith spec (behavior_training_split.md). action_dim
    # stays at the Pi0Config default (32), matching the canonical b1k checkpoints.
    return pi0_config.Pi0Config(pi05=True, action_horizon=32)


def _b1k_frozen_split() -> dict:
    """Load the frozen, versioned train/val episode split (behavior_training_split.md).

    The manifest (b1k_waves/frozen_val_split.json) is generated by
    b1k_waves/create_frozen_val_split.py and defines ~15 held-out episodes/task (~7.5%)
    so every model variant shares an identical train corpus and validation slice.
    Returns None when the manifest is absent (falls back to per-family ledger lists).
    """
    path = pathlib.Path(__file__).parents[3] / "b1k_waves" / "frozen_val_split.json"
    if not path.exists():
        logging.warning("frozen_val_split.json not found at %s; falling back to ledger lists", path)
        return None
...
def _make_wave_configs() -> list[TrainConfig]:
    ...
    # Frozen, versioned validation split (~15 eps/task) shared by every variant so all
    # models train on the identical remaining corpus and validate on the identical slice.
    frozen = _b1k_frozen_split()
    ...
    for w in waves:
        ...
        # Only media inside this wave's demo window is ever staged (stage_wave.py
        # downloads demos [lo,hi) for all 100 tasks), so intersect the shared frozen
        # split with that window. Without this the loader would request the entire
        # corpus on demand (infeasible on this pod's disk). global id = task*200+demo.
        lo, hi = w["demo_lo"], w["demo_hi"]
        train_episodes = (
            [i for i in frozen["train_episodes"] if lo <= i % 200 < hi]
            if frozen else w["train_episodes"]
        )
        val_episodes = (
            [i for i in frozen["val_episodes"] if lo <= i % 200 < hi]
            if frozen else w["val_episodes"]
        )
        configs.append(
            TrainConfig(
                name=name,
...
def _make_family_configs() -> list[TrainConfig]:
    """Build one TrainConfig per super-family, all branching from the backbone.

    Warm-start topology is a STAR, NOT a chain: the backbone foundation inherits
    the wave8 checkpoint; every super-family expert (F4/F3/F2/F1) inherits
    directly from the completed backbone foundation checkpoint. Do NOT chain the
    experts off each other -- sequential chaining causes catastrophic forgetting
    and cross-family skill corruption. The env var B1K_FAMILY_WARM_START_PARAMS
    overrides the default per-family branch (manual intervention only).
    """
    import json

    ledger_path = pathlib.Path(__file__).parents[3] / "b1k_families" / "family_ledger.json"
    if not ledger_path.exists():
        return []
    d = json.loads(ledger_path.read_text())
    families = d.get("families", [])
    wave8_params = d["wave8_final_params"]
    backbone_params = d["backbone_final_params"]

    # Frozen, versioned validation split shared by every family expert so all variants
    # train on the identical remaining corpus and validate on the identical slice.
    frozen = _b1k_frozen_split()
    configs: list[TrainConfig] = []
    for f in families:
        name = f"pi05_b1k_{f['name']}"
        steps = int(f.get("num_train_steps", 15_000))
        # stage_family.py only downloads each family's task_ids within demos [lo,hi),
        # so intersect the shared frozen split with that task set + demo window. The
        # ledger's own train_episodes/val_episodes lists are a legacy contiguous-tail
        # variant and are NOT used when the frozen split exists. global id = task*200+demo.
        lo, hi, tids = f["demo_lo"], f["demo_hi"], set(f["task_ids"])
        train_episodes = (
            [i for i in frozen["train_episodes"] if lo <= i % 200 < hi and (i // 200) in tids]
            if frozen else f["train_episodes"]
        )
        val_episodes = (
            [i for i in frozen["val_episodes"] if lo <= i % 200 < hi and (i // 200) in tids]
            if frozen else f["val_episodes"]
        )
        # Star topology: backbone from wave8, every expert from backbone.
        default_ws = wave8_params if f["name"] == "backbone_foundation" else backbone_params
        configs.append(
            TrainConfig(
                name=name,
                exp_name=f["name"],
                ...
                # Params only (CheckpointWeightLoader never reads train_state/), so
                # optimizer state + LR-schedule position start clean at each stage.
                weight_loader=weight_loaders.CheckpointWeightLoader(
                    os.environ.get("B1K_FAMILY_WARM_START_PARAMS", default_ws)
                ),
                num_train_steps=steps,
                lr_schedule=_optimizer.CosineDecaySchedule(decay_steps=steps),
                freeze_filter=_b1k_wave_model().get_freeze_filter(),
